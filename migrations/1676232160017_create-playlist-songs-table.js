...
exports.up = (pgm) => {
  ...
  pgm.addConstraint(PLAYLIST_SONGS_STR, FK_PLAYLIST_SONGS_PLAYLIST_ID, `
    FOREIGN KEY(playlist_id) REFERENCES ${PLAYLISTS_STR}(id) 
    ON DELETE CASCADE
  `.trim(),
  );

  pgm.addConstraint(PLAYLIST_SONGS_STR, FK_PLAYLIST_SONGS_SONG_ID, `
...
exports.down = (pgm) => {
  pgm.dropConstraint(PLAYLIST_SONGS_STR, FK_PLAYLIST_SONGS_SONG_ID);
  pgm.dropConstraint(PLAYLIST_SONGS_STR, FK_PLAYLIST_SONGS_PLAYLIST_ID);
